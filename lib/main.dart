...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  void _textFieldListener() {
    ...
    void _textFieldListenerUsername() {
    final length = _controller.text.length;
    if (length == _textLength) {
      return;
    }
    _textLength = length;
    if (_valid && _computeProgressUsername(false) < 1) {
      _animateCursor(forward: false);
      return;
    }
    if (_valid) return;
  ...
  late final _expressionsUsername = [
    _lowerCaseRegex,
  ];
  ...
  double _computeProgress([bool shouldUpdateState = true]) {
    ...
    double _computeProgressUsername([bool shouldUpdateState = true]) {
    final text = _controller.text;
    int progress = 0;
    if (text.length >= 8) {
      progress += 1;
    }
    for (final regex in _expressionsUsername) {
      if (regex.hasMatch(text)) {
        progress += 1;
      }
    }
  ...
  void _throttleCursorBlink() {
    ...
    _stopBlinkingCursor();
    _cursorTimer?.cancel();
    _cursorTimer = Timer.periodic(const Duration(milliseconds: 200), (timer) {
      if (_valid) {
        _throttling = false;
        timer.cancel();
        return;
        ...
        child: Column(
          ...
          children: <Widget>[
            Center(
              child: Image.network(
                _valid
                    ? 'https://pbs.twimg.com/media/F4q1-i9WQAE0XUA?format=png&name=360x360'
                    : 'https://pbs.twimg.com/media/F4q1-i-XcAAltyx?format=png&name=360x360',
                width: 200,
              ),
            ),
            const SizedBox(
            ...
            Stack(
              children: [
                ...
                AnimatedPositioned(
                  duration:
                      Duration(milliseconds: _valid || _animateBack ? 100 : 0),
                  left: _offset,
                  top: _height * 0.6,
